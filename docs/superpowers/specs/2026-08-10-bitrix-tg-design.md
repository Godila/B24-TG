...
## Пост-ревью поправки (2026-08)

Спецификация сохранена как исторический документ; ниже — расхождения с
реализацией, зафиксированные по итогам ревью планов 001-008 (2026-08).
Историю не переписываем, сюда — что на самом деле в проде.

1. **§2.1 `/oauth/callback`** — не реализован. OAuth-flow с redirect-URI не
   используется: токены приходят через webhook `ONAPPINSTALL` (сохраняются в
   БД, авто-refresh) либо headless-получением при установке приложения.
   Эндпоинта `/oauth/callback` в web-приложении нет.
2. **§8.1 шаг 8 (WebSocket + Redis pub/sub)** — не реализован и не нужен:
   Web UI обновляется polling-ом (`GET /api/dialogs/{id}/messages?since=`).
   Redis из проекта удалён (Plan 008): не использовался ни одним сервисом.
3. **§8.2 шаги 5-6 (статусы доставки + timeline-запись исходящих)** — закрыты
   планами 005/006: OutboxWorker закрывает `messages.status`
   (`mark_sent`/`mark_failed`, deferral не сжигает попытки), запись в timeline
   идёт через очередь `crm_sync` с retry/backoff.
4. **§9 (MAX)** — реальная оценка L: MessengerProvider как точка расширения
   сохранена, но интеграция MAX не исследована и не начиналась (API MAX
   на 2026-08 не изучалось — см. §11.1). «Готовность к расширению» означает
   только отсутствие блокирующих архитектурных решений, не близость к готовности.
5. **§7.4/§7.5 (`SENTRY_DSN`, Sentry-алерты)** — не реализован: Sentry в проекте
   нет, поля `sentry_dsn` в Settings нет (Plan 008). `B24_OAUTH_REDIRECT` из
   §7.4 также удалён как неиспользуемый (см. поправку 1).
