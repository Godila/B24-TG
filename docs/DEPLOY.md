...
`https://<портал>/marketplace/dev/` (внутри «Приложений» левого меню; если
пункта нет — шестерёнка внизу меню → «Настроить меню» → включить) → в списке
ваших локальных приложений открыть «ЧатМост» (или прямая ссылка
`/marketplace/local/`) → поле «Иконка приложения» ← файл
`src/app/static/brand/chatmost-dark-square-192x192.png` (тёмный вариант —
левое меню Битрикс24 тёмное) → Сохранить. Все размеры и исходники — `assets/logo/`.
Иконка подтягивается не мгновенно (кэш Б24) — перезайти в портал / Ctrl+F5.
