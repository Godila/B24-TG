...
иконку САМОГО приложения. Загружается вручную на портале (нужны права
администратора): раздел «Разработчикам» — прямая ссылка
`https://<портал>/marketplace/dev/` → плитка **«Другое»** → в слайдере
**«Локальное приложение»** → в списке ваше приложение (код вида
`local.<hash>…`; НЕ «Установленные» — это маркетплейс) → поле
«Иконка приложения» ← файл `src/app/static/brand/chatmost-dark-square-192x192.png`
(тёмный вариант — левое меню Битрикс24 тёмное) → Сохранить. Все размеры и
исходники — `assets/logo/`. Иконка подтягивается не мгновенно (кэш Б24) —
перезайти в портал / Ctrl+F5.
