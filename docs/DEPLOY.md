...
`https://<портал>/marketplace/dev/` → вкладка **«Интеграции»** сверху (НЕ
плитка «Другое» — та только создаёт новое; НЕ «Установленные» — это
маркетплейс) → в списке ваше локальное приложение (код вида `local.<hash>…`)
→ форма изменения → поле «Иконка приложения» ← файл
