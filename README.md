...
- 🔁 **Двусторонняя синхронизация с CRM** — входящее → матчинг по номеру → карточка клиента (режим из панели: Контакт+Сделка или только Лид) → канал в поле «Источник» → комментарий в timeline → уведомление менеджеру. Сконвертированный лид сам перепривязывает диалог к сделке.
...
Telegram/MAX → bridge (event) → IncomingHandler
  → crm_sync-очередь → Bitrix24Sync: findbyComm → карточка (Сделка|Лид, режим из панели) → timeline → notify
